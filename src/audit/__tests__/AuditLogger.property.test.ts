...
const auditLogEntryArb = fc.record({
  ...
  actorType: fc.constantFrom('user', 'service') as fc.Arbitrary<'user' | 'service'>,
  timestamp: fc.date().filter(d => !Number.isNaN(d.getTime())).map(d => d.toISOString()),
  ...
  previousState: fc.option(fc.dictionary(fc.string(), fc.anything()), { nil: undefined }),
  newState: fc.option(fc.dictionary(fc.string(), fc.anything()), { nil: undefined }),
});
...
describe('AuditLogger Property Tests', () => {
  ...
  it('Property 1: For any valid AuditLogEntry, deserializeEntry(serializeEntry(entry)) deeply equals the original entry', () => {
    fc.assert(
      fc.property(auditLogEntryArb, (entry) => {
        ...
        expect(deserializeEntry(serializeEntry(entry))).toEqual(normalizedEntry);
      }),
      { numRuns: 100 }
    );
  });
  ...
  it('Property 2: For any ordered sequence of valid events passed to track(), transport batches concatenate to the original sequence', async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(auditLogEntryArb), fc.integer({ min: 1, max: 100 }), async (events, batchSize) => {
        const received: AuditLogEntry[] = [];
        const transport: AuditTransport = async (batch) => {
          received.push(...batch);
        };

        const logger = new AuditLogger({ transport, batchSize, flushIntervalMs: 100000 });
  ...
  it('Property 3: Invalid events do not throw and are not delivered', async () => {
    const invalidEntryArb = fc.record({
      ...
      action: fc.string({ minLength: 1 }),
      actorId: fc.string({ minLength: 1 }),
      actorType: fc.string().filter(s => s !== 'user' && s !== 'service') as any, // Invalid actorType
    });

    await fc.assert(
      fc.asyncProperty(invalidEntryArb, async (invalidEvent) => {
        let called = false;
        const transport: AuditTransport = async () => { called = true; };
        
        const logger = new AuditLogger({ transport, batchSize: 1, flushIntervalMs: 100000 });
    ...
    await fc.assert(
      fc.asyncProperty(
        fc.integer({ min: 1, max: 50 }).chain(batchSize => 
          fc.tuple(fc.constant(batchSize), fc.array(auditLogEntryArb, { minLength: batchSize, maxLength: batchSize }))
        ),
  ...
  it('Property 5: Post-shutdown events never reach transport', async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(auditLogEntryArb, { minLength: 1 }), async (events) => {
        let callCount = 0;
        const transport: AuditTransport = async () => { callCount++; };
