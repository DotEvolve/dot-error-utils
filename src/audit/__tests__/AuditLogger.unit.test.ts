...
import { AuditLogger, ValidationError, deserializeEntry, serializeEntry } from '../AuditLogger';
...
describe('AuditLogger Unit Tests', () => {
  ...
  describe('Constructor Validation', () => {
    ...
    it('throws ValidationError when batchSize < 1', () => {
      expect(() => new AuditLogger({ transport: mockTransport, batchSize: 0 })).toThrow(ValidationError);
      expect(() => new AuditLogger({ transport: mockTransport, batchSize: -1 })).toThrow(ValidationError);
    });

    it('throws ValidationError when flushIntervalMs < 100', () => {
      expect(() => new AuditLogger({ transport: mockTransport, flushIntervalMs: 99 })).toThrow(ValidationError);
    });
  });
  ...
  describe('track() validation', () => {
    ...
    it('discards event with missing required fields', () => {
      logger.track({ tenantId: 't1', actorId: 'a1', actorType: 'user' } as AuditLogEntry); // missing action
    ...
    it('discards event with invalid actorType', () => {
      logger.track({ tenantId: 't1', action: 'a', actorId: 'a1', actorType: 'invalid' as any });
      expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('invalid actorType'));
    });

    it('discards event with invalid timestamp string', () => {
      logger.track({ tenantId: 't1', action: 'a', actorId: 'a1', actorType: 'user', timestamp: 'not-a-date' });
      expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('invalid timestamp'));
    });

    it('auto-generates ISO-8601 timestamp if missing', async () => {
      vi.setSystemTime(new Date('2026-01-01T00:00:00.000Z'));
      logger.track({ tenantId: 't1', action: 'a', actorId: 'a1', actorType: 'user' });
      await logger.flush();
      expect(mockTransport).toHaveBeenCalledWith([{
        tenantId: 't1', action: 'a', actorId: 'a1', actorType: 'user', timestamp: '2026-01-01T00:00:00.000Z'
      }]);
    });
  });
  ...
  describe('Flushing Behavior', () => {
    it('flushes when batchSize is reached', () => {
      const e = { tenantId: 't1', action: 'a', actorId: 'a1', actorType: 'user' as const };
      logger.track(e);
      logger.track(e);
      expect(mockTransport).not.toHaveBeenCalled();
      ...
      expect(mockTransport).toHaveBeenCalledWith([expect.objectContaining(e), expect.objectContaining(e), expect.objectContaining(e)]);
    });
    ...
    it('handles transport rejection without crashing', async () => {
      ...
      expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('transport rejected'), expect.any(Error));
    });
  ...
  describe('shutdown()', () => {
    it('flushes remaining events and prevents future tracks', async () => {
      ...
      logger.track(e);
      expect(console.warn).toHaveBeenCalledWith(expect.stringContaining('called after shutdown'));
  ...
  describe('Serialization & Deserialization', () => {
    it('deserializes valid JSON', () => {
      const e: AuditLogEntry = { tenantId: 't1', action: 'a', actorId: 'a1', actorType: 'user' };
    ...
    it('throws ValidationError for JSON missing required fields', () => {
      expect(() => deserializeEntry(JSON.stringify({ tenantId: 't1' }))).toThrow(ValidationError);
    });
  });
});
